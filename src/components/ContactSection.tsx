...
const screens: Record<"desktop" | "tablet" | "mobile", ScreenBox> = {
  ...
  tablet:  { left: 23.7,  top: 6.95, width: 15.8, height: 80.1 },
  mobile:  { left: 23.15, top: 7.1,  width: 16.2, height: 80.6 },
};
...
const REPLIES = [
  {
    id: "a",
    chip: "Tengo un proyecto 💡",
    response: ["¡Me lo imaginaba! 🔥", "Cuéntame todo 👇"],
  },
  {
    id: "b",
    chip: "Solo espío 👀",
    response: ["Espiar está permitido aquí 👀✨", "¿Qué encontraste?"],
  },
  {
    id: "c",
    chip: "Quiero saber más 🧩",
    response: ["¡Preguntá lo que quieras! 😊", "Estoy por aquí"],
  },
] as const;
...
// Fases de la secuencia automática
// 0 → 1 typing → 2 msg1 → 3 typing → 4 sticker → 5 typing → 6 msg2 + replies
// tras elegir reply: → 7 user msg + typing → 8 respuesta sara + cta
  ...
  return (
    <div className="msg-in flex w-fit items-center gap-[4px] rounded-[14px] rounded-bl-[5px] bg-[#1c262d] px-[10px] py-[8px]">
      {[0, 160, 320].map((delay) => (
        <span
          key={delay}
          className="typing-dot h-[4px] w-[4px] rounded-full bg-white/45"
...
function MessageMeta({ time, sent = false }: { time: string; sent?: boolean }) {
  return (
    <span className="mt-[4px] flex items-center justify-end gap-[2px] text-[7px] leading-none opacity-45">
      {time}
      {sent && (
        <CheckCheck size={8} className="text-[#8fd3ff]" strokeWidth={2.2} />
      )}
    </span>
  );
}
...
function Bubble({
  ...
  return (
    <div
      className={`msg-in relative w-fit cursor-pointer select-none ${sent ? "ml-auto" : ""}`}
      style={{ animationDelay: `${delay}ms` }}
      onClick={onReact}
      title="Toca para reaccionar"
    >
      <div
        className={`max-w-[88%] rounded-[14px] px-[9px] py-[7px] text-[11px] leading-[1.45] shadow-[0_2px_8px_rgba(0,0,0,.2)] transition-transform active:scale-[0.97] ${
          sent
            ? "rounded-br-[4px] bg-[#204b39] text-[#effff6]"
            : "rounded-bl-[4px] bg-[#1c262d] text-[#f2f5f7]"
        }`}
      >
        {children}
      </div>
      {reaction && (
        <span
          key={reaction}
          className="reaction-badge absolute -bottom-[10px] right-[4px] rounded-full bg-[#1a2730] px-[5px] py-[1px] text-[9px] shadow-[0_1px_5px_rgba(0,0,0,.5)]"
        >
          {reaction}
        </span>
      )}
...
// ─── UI de la pantalla ────────────────────────────────────────────────────────
// Siempre se renderiza a DESIGN_WIDTH px; el padre la escala con transform.
function PhoneScreenUI({ onReset }: { onReset: () => void }) {
  ...
  // secuencia automática hasta fase 6
  useEffect(() => {
    if (phase >= 6) return;
  ...
  // scroll al fondo en cada cambio
  useEffect(() => {
    ...
    <>
      <style>{`
        ...
        @keyframes floatHeartA {
          ...
          18% { opacity:.95; }
          100%{ opacity:0; transform:translateY(-22px) translateX(-8px) scale(1.08); }
        }
        @keyframes floatHeartB {
          0%  { opacity:0; transform:translateY(10px) scale(.8); }
          20% { opacity:.94; }
          100%{ opacity:0; transform:translateY(-28px) translateX(8px) scale(1.1); }
        }
        @keyframes floatHeartC {
          0%  { opacity:0; transform:translateY(10px) scale(.76); }
          18% { opacity:.9; }
          100%{ opacity:0; transform:translateY(-18px) translateX(2px) scale(1.02); }
        }
        ...
        @keyframes batteryCharge {
          ...
          25%  { width: 45%; opacity: .72; }
          55%  { width: 72%; opacity: .85; }
          80%  { width: 92%; opacity: .95; }
          90%  { width: 96%; opacity: 1;   }
      ...
      {/* ── Fondo rosa aesthetic — solo en zona de chat ── */}
      {/* blob rosa izquierda */}
      <div
        ...
        style={{
          left: "-12%", top: "28%",
          width: "70%", height: "55%",
          background: "radial-gradient(ellipse at center, rgba(236,72,153,.13) 0%, transparent 68%)",
          filter: "blur(18px)",
        }}
      />
      {/* blob durazno derecha */}
      <div
        className="pointer-events-none absolute z-[1]"
        style={{
          right: "-10%", top: "42%",
          width: "60%", height: "45%",
          background: "radial-gradient(ellipse at center, rgba(251,113,133,.09) 0%, transparent 65%)",
          filter: "blur(14px)",
        }}
      />
      {/* toque magenta sutil centro-alto */}
      <div
        className="pointer-events-none absolute z-[1]"
        style={{
          left: "20%", top: "22%",
          width: "55%", height: "30%",
          background: "radial-gradient(ellipse at center, rgba(217,70,239,.06) 0%, transparent 70%)",
          filter: "blur(22px)",
        }}
      />
      ...
      <div className="relative z-10 flex items-center justify-between px-[5%] pb-[1.5%] pt-[7%] text-white/85">
        ...
        <div className="flex items-center gap-[5px] text-white/68">
          ...
          <span className="inline-flex items-end gap-[1px]">
            {[3, 5, 7, 9].map((h) => (
              <i key={h} className="w-[2px] rounded-sm bg-current" style={{ height: h }} />
            ))}
          </span>
          {/* wifi */}
          ...
          <span className="relative inline-flex h-[8px] w-[15px] items-center rounded-[3px] border border-white/48 p-[1px]">
            ...
            {/* rayo de carga — parpadea en sincronía */}
      ...
      <div className="relative z-10 border-b border-white/[0.05] bg-[#111b21]/96 px-[4%] py-[2.4%] backdrop-blur-sm">
        <div className="flex items-center gap-[6px]">
          <button
            aria-label="Volver"
            className="grid h-[22px] w-[22px] place-items-center rounded-full bg-white/[0.06] text-white/75 transition hover:bg-white/10"
          >
            <ArrowLeft size={11} />
          </button>
          ...
          <div className="min-w-0 flex-1">
            ...
            <p
              className={`mt-[2px] text-[9px] leading-none transition-colors duration-300 ${
                phase === 7 ? "text-[#25D366]/80" : "text-white/50"
              }`}
            >
          ...
          <div className="flex shrink-0 items-center gap-[5px] text-white/60">
            {([Video, Phone, MoreVertical] as const).map((Icon, i) => (
              <button
                key={i}
                className="grid h-[22px] w-[22px] place-items-center rounded-full bg-white/[0.06] transition hover:bg-white/10"
              >
                <Icon size={10} />
              </button>
            ))}
      ...
      <div
        ref={chatRef}
        className="chat-scroll relative z-10 min-h-0 flex-1 overflow-y-auto px-[4.5%] py-[3%]"
      >
        <div className="flex min-h-full flex-col justify-end gap-[8px]">
          ...
          {phase >= 4 && (
            <div className="msg-in relative w-fit max-w-[52%]">
              ...
              <img
                ...
                className="block h-auto max-h-[72px] w-auto rounded-[8px] object-contain"
                loading="lazy"
                decoding="async"
              />
              <span className="mt-[3px] block text-right text-[7px] leading-none text-white/36">
                9:41
              </span>
            </div>
          )}
            ...
            <div className="flex flex-col gap-[5px] pl-[1px]">
              {REPLIES.map((r, i) => (
                <button
                  ...
                  className="reply-chip reply-in w-fit rounded-full border border-white/20 bg-white/[0.06] px-[10px] py-[5px] text-left text-[10px] text-white/78"
          ...
          {/* Respuesta de Sara + CTA */}
          {phase === 8 && pickedReply && (
            <>
              <Bubble reaction={reactions.m8} onReact={() => handleReact("m8")}>
                {pickedReply.response[0]}
                <br />
                {pickedReply.response[1]}
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="heartbeat-cta msg-in ml-[1px] inline-flex w-fit items-center gap-[6px] rounded-full bg-[#25D366] px-[11px] py-[6px] text-[10px] font-semibold text-[#04140d] transition hover:scale-[1.02] active:scale-[0.97]"
              ...
              <button
                onClick={onReset}
                className="reset-btn msg-in ml-[1px] inline-flex w-fit items-center gap-[5px] rounded-full border border-white/14 bg-white/[0.05] px-[9px] py-[5px] text-[9px] text-white/45 transition"
      ...
      <div className="relative z-10 mt-auto border-t border-white/[0.05] bg-[#0f171d]/96 px-[4%] pb-[3%] pt-[2.2%] backdrop-blur-sm">
        <div className="flex items-center gap-[4px]">
          <button
            aria-label="Adjuntar"
            className="grid h-[24px] w-[24px] place-items-center rounded-full bg-white/[0.06] text-white/70"
          >
            <Plus size={9} />
          </button>
          <div className="flex h-[24px] flex-1 items-center rounded-full bg-[#1d2a32] px-[8px] text-[9px] text-white/32">
            Escribe un mensaje…
          </div>
          <button
            aria-label="Cámara"
          ...
          <button
            ...
            className="grid h-[24px] w-[24px] place-items-center rounded-full bg-[#25D366] text-black shadow-[0_5px_14px_rgba(37,211,102,.3)]"
          >
            <Mic size={9} />
          </button>
        </div>
...
export default function ContactSection() {
  ...
  const { width: screenWidth, height: screenHeight } = useElementSize(
    screenRef as React.RefObject<HTMLElement>
  );

  const activeScreen = useMemo(() => {
    if (viewportWidth <= 640)  return screens.mobile;
    if (viewportWidth <= 1024) return screens.tablet;
    return screens.desktop;
  }, [viewportWidth]);

  // key para forzar remount completo al reiniciar
    ...
    <section id="contact" className="relative w-full overflow-hidden bg-[#050505]">
      <div className="mx-auto w-full max-w-[1800px] px-2 md:px-4 xl:px-6">
        <div className="relative isolate mx-auto w-full max-w-[1440px] min-w-[320px]">
          ...
          {/* Espaciador para dar altura al contenedor */}
          <img
            src="/Assets/Contactame/AvatarContactame.svg"
            alt=""
            aria-hidden
            className="pointer-events-none block w-full min-w-[320px] max-w-[1440px] select-none opacity-0"
            loading="lazy"
            decoding="async"
          />
